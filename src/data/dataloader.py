...
class MSGVIPDataset(Dataset):
    ...
    def _extract_sample_data(self, data: np.lib.npyio.NpzFile, file_path: str) -> Dict[str, Any]:
        ...
        source_h = int(frame_tensor.shape[1])
        source_w = int(frame_tensor.shape[2])
        if source_h <= 0 or source_w <= 0:
            raise ValueError(f"Invalid frame shape for bbox normalization: H={source_h}, W={source_w}")
        ...
        # Normalize bboxes using actual frame size from the sample.
        # This avoids hidden dependence on config.image_height/image_width.
        bbox_tensor[..., [0, 2]] = bbox_tensor[..., [0, 2]].clamp(0, float(source_w)) / float(source_w)
        bbox_tensor[..., [1, 3]] = bbox_tensor[..., [1, 3]].clamp(0, float(source_h)) / float(source_h)
